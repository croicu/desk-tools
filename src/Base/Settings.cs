...
/// <c>settings.local.json</c>'s own <c>"settings"</c> object overriding it key-by-key (a personal,
/// gitignored override file -- see .gitignore). Load once via <see cref="Load"/>, then read back
/// via <see cref="Current"/>. Implements <see cref="ISettingsProvider"/> so a caller that accepts
/// that interface works unmodified with a real, file-backed instance.
/// </summary>
public sealed class Settings : ISettingsProvider
{
    ...
    private Settings(bool debug, TelemetryLevel logging, List<string> logCategories, List<string> excludedCategories)
    {
        Debug = debug;
        LogLevel = logging;
    ...
    public static Settings Load(string? path = null, string? localPath = null)
    {
        var debug = false;
        ...
        var payload = LoadPayload(path ?? DefaultSettingsPath, localPath ?? DefaultLocalPath);
        ...
        var settings = new Settings(debug, logLevel, logCategories, excludedCategories);
        CurrentInstance.Value = settings;
        return settings;
    }

    /// <summary>
    /// Returns one named top-level section of settings.json (merged with settings.local.json's
    /// override), e.g. a plugin's or subsystem's own config section living as a sibling of the
    /// core "settings" object, not nested inside it. settings.json's shape is uniform across
    /// consumer repos, so this is the public entry point for a caller outside this class to reach
    /// its own section without knowing how the file is read or merged -- that stays encapsulated
    /// in <see cref="LoadTopLevel"/> (distinct from <see cref="LoadPayload"/>, which only reads
    /// the "settings" object itself for <see cref="Load"/>'s own use).
    /// </summary>
    public static Dictionary<string, JsonElement> Section(string name, string? path = null, string? localPath = null)
    {
        var topLevel = LoadTopLevel(path ?? DefaultSettingsPath, localPath ?? DefaultLocalPath);
    ...
    /// <summary>
    /// Reads settings.json's "settings" object, with settings.local.json's own "settings" object
    /// overriding it key-by-key. Private -- Section() is the public way for a caller outside this
    /// class to reach a section of the merged file.
    /// </summary>
    private static Dictionary<string, JsonElement> LoadPayload(string path, string localPath)
    {
        ...
        if (File.Exists(path))
        {
            ...
            {
                throw new SettingsError("settings.json must contain a JSON object.");
            }
            ...
            if (doc.RootElement.TryGetProperty("settings", out var settingsEl))
            {
                ...
                {
                    throw new SettingsError("'settings' in settings.json must be a JSON object.");
                }
            ...
            Logger.Diagnostic($"settings file found and parsed: '{path}'", DiagnosticsCategories.Settings);
        }
        else
        {
            // Load()'s own consumer effect: with no settings.json, every knob it reads (debug,
            // logLevel, logCategories, excludedCategories) falls back to its own restrictive
            // default -- a meaningful behavior change (e.g. Verbose/Info logging is unreachable for
            // the rest of the run without one), so this is Warning rather than Info.
            Logger.Warning($"settings file '{path}' not found; falling back to restrictive defaults (debug=false, logLevel=error).", DiagnosticsCategories.Settings);
        }
        ...
        if (File.Exists(localPath))
        {
            ...
            {
                // Unlike a malformed settings.json (which throws), a malformed settings.local.json
                // is silently ignored by design -- see the caller-facing shape check above. Silent
                // was the wrong call by itself (a typo'd override would otherwise vanish with zero
                // signal), so surface it as a Warning instead of leaving it truly silent.
    ...
    /// <summary>
    /// Reads settings.json's actual top-level properties (the "settings" object itself included,
    /// as just another entry), with settings.local.json's own top-level properties overriding them
    /// key-by-key. Distinct from <see cref="LoadPayload"/>, which reaches inside "settings" instead
    /// of returning it whole -- <see cref="Section"/> is the only caller of this method.
    /// </summary>
    private static Dictionary<string, JsonElement> LoadTopLevel(string path, string localPath)
    {
        var payload = new Dictionary<string, JsonElement>();
        ...
        {
            // Section()'s own consumer effect is narrower than Load()'s -- a missing file just
            // means whatever section the caller asked for comes back empty, not a run-wide behavior
            // change -- so Info rather than Load()'s Warning for the same missing-file condition.
            Logger.Info($"settings file '{path}' not found; {nameof(Section)} returns an empty section.", DiagnosticsCategories.Settings);
        }
