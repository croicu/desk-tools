...
public sealed class ErrorsTests
{
    ...
    /// ConsoleLog (see Diagnostics.cs) if nothing else has already; nothing here ever disposes
    /// it, so reset after each test to avoid leaving a live default that could make some other
    /// test's own ConsoleLog.Create() throw.
    /// </summary>
    [TestCleanup]
    ...
    /// <summary>
    /// Logger.Flush()/Clear() touch DiagnosticsLog's Pending buffer, but that buffer is now
